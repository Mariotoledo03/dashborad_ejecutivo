...
with col1:
    periodo = st.selectbox("📅 Periodo de análisis:", ["Últimos 30 días", "Último trimestre", "Último año", "Todo el periodo"])
...
if df['usuarios_activos'].tail(1).iloc[0] > df['usuarios_activos'].quantile(0.9):
    alertas.append({'tipo': '🎉 Éxito', 'mensaje': 'Usuarios activos en top 10% histórico', 'color': 'green'})

for alerta in alertas:
    st.markdown(f"""
    <div style="padding: 1rem; margin: 0.5rem 0; background-color: {alerta['color']};
                color: white; border-radius: 10px; fontweight: bold;">
